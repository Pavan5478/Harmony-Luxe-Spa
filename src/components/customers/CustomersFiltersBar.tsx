...
function DateChipPicker({
  ...
  const chipText = value ? value : label;
  ...
  return (
    <div className="relative" ref={anchorRef}>
      <button
        ...
        className="inline-flex h-10 items-center gap-2 rounded-full border border-border bg-background px-3 text-sm shadow-sm transition hover:bg-card focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40"
      ...
      {/* hidden native input => NO double calendar icons */}
      <input
        type="date"
        value={value}
      ...
      {open ? (
        <div
          ...
          className="absolute left-0 top-full z-50 mt-3 w-[360px] max-w-[calc(100vw-2rem)] overflow-hidden rounded-2xl border border-border bg-card shadow-card"
        >
...
export default function CustomersFiltersBar(props: {
  initialQ: string;
  initialFrom: string;
  initialTo: string;
  initialStatus: Status;
  count: number;
  showAdminRow: boolean;
  adminRow?: React.ReactNode;
}) {
  const { initialQ, initialFrom, initialTo, initialStatus, count, showAdminRow, adminRow } = props;
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-3 shadow-sm sm:p-4">
      <form onSubmit={onApply} className="min-w-0">
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-2 text-[11px] text-muted">
          <div>
            Showing <span className="font-semibold text-foreground">{count}</span> customers
          </div>
          <div className="hidden sm:block">Tip: Phone is the primary key.</div>
        </div>
      </form>

      {showAdminRow ? <div className="mt-3">{adminRow}</div> : null}
    </div>
  );
}